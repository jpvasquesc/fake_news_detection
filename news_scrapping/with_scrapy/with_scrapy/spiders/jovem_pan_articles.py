...
class JovemPanArticlesSpider(scrapy.Spider):
    """
    Web scrapping 'spider', based on scrapy's 'Spider' class. 
    Built to scrap the text of news articles under the 'jovempan.com.br' domain.   
    """
    name = 'jovem_pan_articles'
    allowed_domains = ['jovempan.com.br']

    # Get links to the news articles from which the text will be scrapped
    links_path = Path("../../../links/jovem_pan.txt")
    ...
    def parse(self, response):
        ...
        PARAGRAPH_SELECTOR = "div.context > *:not([class*='list-related-posts']):not([class*='post_image'])"
        TEXT_SELECTOR = ".//text()"
        ...
        article_file = Path("../../../articles/jovem_pan/{0}.txt".format(file_name))
